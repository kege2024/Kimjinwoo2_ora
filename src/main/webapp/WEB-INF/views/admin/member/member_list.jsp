		...
		<div class="card">
			...
			<div class="card-body table-responsive p-0">
				<table class="table table-hover text-nowrap">
					...
					<tbody>
						<c:forEach items="${memberList}" var="memberVO" varStatus="status">
							<tr>
								...
								<td><a
									href="/admin/member/view?user_id=${memberVO.user_id}&page=${pageVO.page}">${memberVO.user_name}[${memberVO.point}]</a></td>
								<td>${memberVO.email}</td>
								<td><span class="tag tag-success">${memberVO.enabled}</span></td>
								<td><fmt:formatDate pattern="yyyy년-MM월-dd일 HH:mm" value="${memberVO.reg_date}"/></td>
					...
					<td>
					<nav aria-label="Contacts Page Naviagtion">
							<ul class="pagination" style="position: relative; left: 40%;">
								...
								<c:forEach begin="${pageVO.startPage}" end="${pageVO.endPage}"
									var="idx">
									<li
										class='page-item <c:out value="${idx==pageVO.page?'active':''}"/>'><a
										href="/admin/member/list?page=${idx}&searchType=${pageVO.searchType}&searchKeyword=${pageVO.searchKeyword}" class="page-link">${idx}</a></li>
								</c:forEach>
